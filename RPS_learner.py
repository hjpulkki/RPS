...
def simple_model():
    ...
    new_model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy', 'categorical_crossentropy', 'precision', 'recall'])
    return new_model
...
def batch_generator(filename):
    ...
    with open('data.txt', 'r') as data_file:
        for line in data_file:
            ...
            temp = np_utils.to_categorical(data_vector, nb_classes=3)
...
for i in range(EPOCH_NP):    # manual epochs
    for (input_data, output_data) in batch_generator('data.txt'):
            model.fit(input_data, output_data, nb_epoch=1, batch_size=100)
